...
public class TileEntityFluidPumpAlternative extends blusunrize.immersiveengineering.common.blocks.metal.TileEntityFluidPump implements ITickable, IBlockBounds, IHasDummyBlocks, IConfigurableSides, IFluidPipe, IIEInternalFluxHandler, IBlockOverlayText, IPlayerInteraction {
    ...
    @Override public void readCustomNBT(@Nonnull NBTTagCompound nbt, boolean descPacket) {
        ...
        if (nbt.hasKey("searchFluid")) { searchFluid = FluidRegistry.getFluid(nbt.getString("searchFluid")); }
        else { searchFluid = null; }
    ...
    public void drainFromTank(IFluidHandler handler) {
        int rate = ICCommonConfig.pipes.transferRate;
        int pressurizedRate = ICCommonConfig.pipes.pressurizedTransferRate;
    ...
    public int outputFluid(@Nonnull FluidStack fs, boolean simulate) {
        ...
        for (EnumFacing f : EnumFacing.values()) {
            ...
            if (tile instanceof IICPipe && this.energyStorage.extractEnergy(accelPower, true) >= accelPower) {
        ...
        if (sum > 0) {
            ...
            for (DirectionalFluidOutput output : sorting.keySet()) {
                ...
                if (output.containingTile instanceof IICPipe && this.energyStorage.extractEnergy(accelPower, true) >= accelPower) {
    ...
    public void flipFillMode(EntityPlayer player) {
        ...
        ICUtils.sendServerNoSpamMessages(player, new TextComponentTranslation(ICLib.CHAT_INFO + (fillFirstMode ? "fluidPump.fillFirst" : "fluidPump.pushOnly")));
    }
