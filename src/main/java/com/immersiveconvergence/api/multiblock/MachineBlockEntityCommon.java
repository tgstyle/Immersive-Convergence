...
@SuppressWarnings({"unused", "RedundantSuppression"}) public class MachineBlockEntityCommon<State extends IMultiblockState> implements ISubmodelOffsetProvider {
    ...
    public MachineBlockEntityCommon(MultiblockRegistration<State> multiblock, Supplier<IMultiblockBEHelper<State>> helperSupplier, Supplier<Level> levelSupplier) {
    ...
    @Override public BlockPos getModelOffset(BlockState state, Vec3i size) {
        ...
        if (state != null && state.hasProperty(IEProperties.MIRRORED) && state.getValue(IEProperties.MIRRORED)) {
            Level level = levelSupplier.get();
            if (level != null) {
                Vec3i realSize = multiblock.size(level);
                if (realSize != null) {
