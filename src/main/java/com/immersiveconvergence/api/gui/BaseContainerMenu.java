...
@Mod.EventBusSubscriber(modid = ICLib.MODID, bus = Mod.EventBusSubscriber.Bus.FORGE)
public abstract class BaseContainerMenu extends AbstractContainerMenu {
    protected final List<MenuSyncData<?>> genericData = new ArrayList<>();
    ...
    protected BaseContainerMenu(MenuContext ctx) {
        super(ctx.type, ctx.id);
        this.setChanged = ctx.setChanged;
        this.isValid = ctx.isValid;
    }

    public void addGenericData(MenuSyncData<?> newData) { genericData.add(newData); }
    ...
    @Override public void broadcastChanges() {
        ...
        List<Pair<Integer, MenuSyncSerializers.DataPair<?>>> toSync = new ArrayList<>();
        for (int i = 0; i < genericData.size(); i++) {
            MenuSyncData<?> data = genericData.get(i);
            if (data.needsUpdate()) { toSync.add(Pair.of(i, data.dataPair())); }
        }
        if (!toSync.isEmpty()) { for (ServerPlayer player : usingPlayers) { PacketHandler.sendToPlayer(player, new MessageMenuSync(toSync)); } }
    }

    public void receiveSync(List<Pair<Integer, MenuSyncSerializers.DataPair<?>>> synced) {
        for (Pair<Integer, MenuSyncSerializers.DataPair<?>> syncElement : synced) { genericData.get(syncElement.getFirst()).processSync(syncElement.getSecond().data()); }
    }
    ...
    @Override public void clicked(int id, int dragType, @Nonnull ClickType clickType, @Nonnull Player player) {
        ...
        if (dragType == 2) { slot.set(ItemStack.EMPTY); }
            ...
            if (stackSlot.isEmpty()) {
                if (!stackHeld.isEmpty() && slot.mayPlace(stackHeld)) { slot.set(ItemHandlerHelper.copyStackWithSize(stackHeld, amount)); }
            }
    ...
    @Override @Nonnull public ItemStack quickMoveStack(@Nonnull Player player, int slot) {
        ...
        if (slotObject.hasItem()) {
            ...
            if (itemStack1.isEmpty()) { slotObject.set(ItemStack.EMPTY); }
            else { slotObject.setChanged(); }
        }
        return itemStack;
    }

    protected boolean moveItemStackToWithMayPlace(ItemStack pStack, int pEndIndex) { return moveItemStackToWithMayPlace(slots, this::moveItemStackTo, pStack, 0, pEndIndex); }
    ...
    @SubscribeEvent public static void onContainerOpened(PlayerContainerEvent.Open ev) {
        if (ev.getContainer() instanceof BaseContainerMenu container && ev.getEntity() instanceof ServerPlayer serverPlayer) {
            container.usingPlayers.add(serverPlayer);
            List<Pair<Integer, MenuSyncSerializers.DataPair<?>>> list = new ArrayList<>();
            for (int i = 0; i < container.genericData.size(); i++) { list.add(Pair.of(i, container.genericData.get(i).dataPair())); }
            PacketHandler.sendToPlayer(serverPlayer, new MessageMenuSync(list));
        }
    }

    @SubscribeEvent public static void onContainerClosed(PlayerContainerEvent.Close ev) {
        if (ev.getContainer() instanceof BaseContainerMenu container && ev.getEntity() instanceof ServerPlayer serverPlayer) { container.usingPlayers.remove(serverPlayer); }
    }
    ...
    public static MenuContext blockCtx(MenuType<?> pMenuType, int pContainerId, BlockEntity be) {
        return new MenuContext(pMenuType, pContainerId, () -> {
            be.setChanged();
            if (be instanceof BaseBlockEntity baseBE) { baseBE.markContainingBlockForUpdate(null); }
        }, p -> {
    ...
    public static MenuContext clientCtx(MenuType<?> pMenuType, int pContainerId) { return new MenuContext(pMenuType, pContainerId, () -> {}, $ -> true); }
