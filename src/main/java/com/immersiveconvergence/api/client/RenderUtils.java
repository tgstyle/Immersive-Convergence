...
public class RenderUtils {
    ...
    private static int getLightValue(int[] neighbourBrightness, float[] normalizationFactors, int localBrightness) {
        ...
        if (RenderUtils.quadNormal.x() > 0.0F) {
            sideBrightness = RenderUtils.quadNormal.x() * neighbourBrightness[5];
            type |= 1;
        }
        else { sideBrightness = -RenderUtils.quadNormal.x() * neighbourBrightness[4]; }
        if (RenderUtils.quadNormal.y() > 0.0F) {
            sideBrightness += RenderUtils.quadNormal.y() * neighbourBrightness[1];
            type |= 2;
        }
        else { sideBrightness += -RenderUtils.quadNormal.y() * neighbourBrightness[0]; }
        if (RenderUtils.quadNormal.z() > 0.0F) {
            sideBrightness += RenderUtils.quadNormal.z() * neighbourBrightness[3];
            type |= 4;
        }
        else { sideBrightness += -RenderUtils.quadNormal.z() * neighbourBrightness[2]; }
