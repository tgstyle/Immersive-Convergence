...
@SuppressWarnings({"unused", "RedundantSuppression"}) public class MirrorBakedModel<T extends BakedModel> extends BakedModelWrapper<T> {
    ...
    public MirrorBakedModel(T base) {
        super(base);
        ...
        if (isDynamic) {
            ...
            this.keyProvider = kp;
            this.quadCache = CacheBuilder.newBuilder().maximumSize(1024).expireAfterAccess(10, TimeUnit.MINUTES).build(CacheLoader.from(k -> MirrorModelLoader.reversedQuads(kp.getQuads(k))));
            this.unculledQuads = null;
            this.culledQuads = null;
        }
        else if (base instanceof SimpleBakedModel simple) {
            this.unculledQuads = MirrorModelLoader.getReversedQuads(simple, null);
            EnumMap<Direction, List<BakedQuad>> map = new EnumMap<>(Direction.class);
            for (Direction d : DirectionUtils.VALUES) { map.put(d, MirrorModelLoader.getReversedQuads(simple, d)); }
