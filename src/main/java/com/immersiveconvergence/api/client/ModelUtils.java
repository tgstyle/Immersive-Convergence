...
public class ModelUtils {
    ...
    public static class ITBakedQuadBuilder {
        ...
        private static final int POS_OFFSET = getOffset(VertexFormatElement.POSITION) / 4;
        private static final int COLOR_OFFSET = getOffset(VertexFormatElement.COLOR) / 4;
        private static final int UV_OFFSET = getOffset(VertexFormatElement.UV) / 4;
        private static final int LIGHTMAP_OFFSET = getOffset(VertexFormatElement.UV2) / 4;
        private static final int NORMAL_OFFSET = getOffset(VertexFormatElement.NORMAL) / 4;
        ...
        public void putVertexData(Vec3 pos, Vec3 faceNormal, double u, double v, TextureAtlasSprite sprite, float[] colour, float alpha) {
            ...
            data[base + UV_OFFSET]     = Float.floatToIntBits(sprite.getU((float) (u / 16)));
            data[base + UV_OFFSET + 1] = Float.floatToIntBits(sprite.getV((float) (v / 16)));
