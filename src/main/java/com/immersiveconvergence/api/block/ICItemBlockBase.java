...
public class ICItemBlockBase extends ItemBlock {
    public ICItemBlockBase(Block b) {
        super(b);
    ...
    @Override public void addInformation(@Nonnull ItemStack stack, World worldIn, @Nonnull List<String> list, @Nonnull ITooltipFlag advInfo) {
        if (((ICBlockBase<?>) block).hasFlavour(stack)) {
            ...
            String flavourKey = "desc." + ((ICBlockBase<?>) this.block).context.modid + ".flavor." + ((ICBlockBase<?>) this.block).name + "." + subName;
    ...
    @Override @Nonnull public EnumActionResult onItemUse(@Nonnull EntityPlayer player, @Nonnull World world, @Nonnull BlockPos pos, @Nonnull EnumHand hand, @Nonnull EnumFacing side, float hitX, float hitY, float hitZ) {
        ...
        if (stack.getCount() > 0 && player.canPlayerEdit(pos, side, stack) && canBlockBePlaced(world, pos, side, stack)) {
            ...
            IBlockState iBlockState = this.block.getStateForPlacement(world, pos, side, hitX, hitY, hitZ, i, player);
            if (placeBlockAt(stack, player, world, pos, side, hitX, hitY, hitZ, iBlockState)) {
