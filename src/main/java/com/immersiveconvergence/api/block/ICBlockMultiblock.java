...
public abstract class ICBlockMultiblock<E extends Enum<E> & ICBlockBase.IBlockEnum> extends ICBlockTileProvider<E> {
    ...
    public ICBlockMultiblock(BlockContext context, String name, Material material, PropertyEnum<E> mainProperty, Class<? extends ItemBlock> itemBlock, Object... additionalProperties) {
        super(context, name, material, mainProperty, itemBlock, appendSplitProperty(combineProperties(additionalProperties)));
    ...
    @Override public boolean removedByPlayer(@Nonnull IBlockState state, @Nonnull World world, @Nonnull BlockPos pos, @Nonnull EntityPlayer player, boolean willHarvest) {
        if (!willHarvest) {
            ...
            if (te instanceof TileEntityTemplateMultiblock) {
                TileEntityTemplateMultiblock<?, ?, ?> tile = (TileEntityTemplateMultiblock<?, ?, ?>) te;
